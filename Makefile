...
.PHONY: db-rollback
db-rollback: ## Rollback database migrations (requires DATABASE_URL)
...
.PHONY: db-reset
db-reset: ## Reset database (DANGER: This will drop all data)
...
.PHONY: db-generate
db-generate: ## Generate SQLC code from SQL queries
	@echo "Generating SQLC code..."
	@if command -v sqlc >/dev/null 2>&1; then \
		sqlc generate; \
	else \
		echo "sqlc not found. Please install it:"; \
		echo "  https://sqlc.dev/"; \
		exit 1; \
	fi
